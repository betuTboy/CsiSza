...
def selectmove(allvalidwordscores):
    """Az érvényes szavak közül kiválasztja a megfelelőt, majd normál játékban a finishmove() függvényt hívja"""
    global moveofai
    sortedlist = sorted(allvalidwordscores, key=itemgetter(6), reverse=True)
    percentage = sortedlist[0][6] * strength / 100
    choice1 = []
    for i in range(len(sortedlist)):
        if sortedlist[i][6] < percentage:
            break
    for validword in sortedlist:
